...
- [ ] 1. プロジェクト初期設定とFirebase統合
  ...
  - [x] 1.3 Firestore Security Rulesの実装
...
- [ ] 2. データモデルとリポジトリレイヤーの実装
  ...
  - [x] 2.6 ParticipantRepositoryの実装
    - 参加者のCRUD操作を実装
...
- [ ] 10. Web Interface - 基本構造とFirebase統合
  ...
  - [x] 10.2 FirestoreClientクラスの実装（JavaScript）
    - Firestore初期化ロジックを実装
...
- [ ] 11. Web Interface - スワイプ式タスク選択UI
  - [x] 11.1 ParticipantNameInputコンポーネントの実装
    - 名前入力フォームを実装
    - LocalStorageへの保存を実装
    - _Requirements: 4.6_

  - [x] 11.2 TaskCardViewコンポーネントの実装
    - カード形式のタスク表示を実装
...
- [ ] 12. Web Interface - ステータス更新パネル
  - [x] 12.1 StatusUpdatePanelコンポーネントの実装
    - 自分が引き受けたタスクの一覧表示
    - 完了チェックボックスを実装
    - メモ入力フィールドを実装
    - 支払い完了ボタンを実装
...
- [ ] 16. セキュリティとプライバシーの強化
  - [x] 16.1 Firebase Authenticationの実装
    - 匿名認証を実装
